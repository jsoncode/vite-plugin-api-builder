 ...
 * @param type
 */
export const getSingleType = (type = '') => {
    return typeMap[type] || type;
};
// 提取被引用的类型
export const getReferenceType = (k) => {
...
export const getRefTypeApiFox = (key = '', schemaList) => {
    if (!key) {
        return '';
    }
    let match = '';
...
export async function getSwaggerDoc(options) {
...
export function buildApiFox(apiList, schemaList, config) {
    const fnMap = {};
    const req = {};
    apiList.forEach(i => {
...
export function buildPathTypeAndFn(paths, basePath, filter) {
    const dtoMap = {};
    const fn = {};
...
export function buildSchemaType(definitions) {
    const dtoValue = {};
    const dtoMap = {};
    // 处理公共数据模型
...
export function saveFn(props) {
    ...
    writeFileSync(url, `${importType.join(lineN) + lineN}${Object.values(fn).map((i) => i.fnBody).join('')}`, { encoding: 'utf-8' });
    return importInFn;
}
export function buildImportStr(list) {
...
export function saveTyped(props) {
    ...
    writeFileSync(otherUrl, needUseTypeList.map(i => dtoMap[i]).join(`${lineN}${lineN}`), { encoding: 'utf-8' });
    writeFileSync(dtoValueUrl, needUseTypeList.map(i => dtoValue[i]).filter(Boolean).join(`${lineN}${lineN}`), { encoding: 'utf-8' });
}
...
export function getFormType({ key, list }) {
...
export const getApiAndBuildApiFox = async (config) => {
...
export const getApiAndBuild = async (domain, pathname, config) => {
    if (pathname) {
