...
export const defaultNullData: SingleType[] = [void 0, 'undefined', 'null', null, '']
...
export const setLoading = (type: 'show' | 'close'): void => {
}

export function listenEvent(listenProps: ListenEventProps) {
    const { xhr, url, callback, uploadProgress, downloadProgress, showLoading, showErrorMessage } = listenProps
    // 上传进度回调
    if (uploadProgress) {
...
// 遍历对象中的每个属性，并执行回调函数
export function forEachObj(obj: ObjAny, callback: (value: any, key: string) => void) {
    // 遍历对象中的每个属性
...
export function setHeaders(xhr: XMLHttpRequest, headers: ObjAny) {
    // 遍历headers对象
...
export function clearNullData(params: any, clearList: any[] = [undefined, 'undefined', 'null', '', null]) {
...
export function queryToString(query: MergeParams['query']) {
	const queryList: any[] = []
	...
	.map(i => {
		// 将参数添加到queryList数组中
		const value: SingleType = query[i]
...
// 导出一个函数，用于合并参数
export function mergeParams(params: MergeParams): string {
 ...
 */
export function getSearchParams(url?: string, type: 'search' | 'hash' | 'url' = 'search'): { [key: string]: any } {
	// 定义一个空字符串，用于存储查询字符串
	let queryString = ''
	// 如果传入了url参数
	if (url) {
	...
	if (list) {
		...
		list.forEach(i => {
			// 使用正则表达式匹配参数名和参数值
			const [_, key, value]: any = i.match(new RegExp(regStr))
			// 如果参数值不为undefined
			if (value !== 'undefined') {
				// 对参数值进行解码
...
export function passCode(code?: number) {
	// 判断传入的code是否为0
	return code === 0
}

export function formatCookie(cookieMap:ObjAny){
    const cookie = []
