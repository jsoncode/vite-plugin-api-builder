...
export async function http(url, options = {}) {
	...
	// 解构options中的参数
	const {
		method = 'POST', xhrAttribute = {}, uploadProgress, downloadProgress, showLoading, showErrorMessage = true
	} = options
	let {headers = {}} = options
	...
	if (optBody && (method === 'POST' || method === 'PUT')) {
		...
		headers = {
			...headers, ...headerInParams,
			'Content-Type': 'application/json', ...(cookieInParams ? {cookie: formatCookie(cookieInParams)} : {})
		}
	} else {
		// 否则，将optBody转换为query参数，并拼接到url中
		url = mergeParams({
			url, query: clearNullData(optBody)
		})
	}
	...
	return new Promise(resolve => {
		...
		// 监听xhr的各种事件
		listenEvent({
			xhr, url, uploadProgress, downloadProgress, showLoading, showErrorMessage, callback: data => resolve(data)
		})
		// 发送请求
...
export async function request(url, options = {}) {
	...
	options.headers = {
		authorization: `Bearer ${localStorage.getItem('token')}`, tenantId: '', ...(options.headers || {})
	}
	// 发送请求
