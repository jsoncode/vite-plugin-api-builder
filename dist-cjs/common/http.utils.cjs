...
exports.setLoading = exports.alertError = exports.defaultNullData = void 0;
exports.listenEvent = listenEvent;
exports.forEachObj = forEachObj;
exports.setHeaders = setHeaders;
exports.clearNullData = clearNullData;
...
exports.defaultNullData = [void 0, 'undefined', 'null', null, ''];
...
function listenEvent(listenProps) {
    ...
    xhr.onerror = async () => {
        ...
        showLoading && (0, exports.setLoading)('close');
        showErrorMessage && (0, exports.alertError)(msg);
        ...
        try {
            ...
            if (type?.includes('/json')) {
                if (xhr.response instanceof Blob) {
                    ...
                    // size?: number;
                    // total?: number;
                    const { code, msg, message, data, total, current, pages, size } = JSON.parse(xhr.response);
                    if (passCode(code)) {
                        callback({
                            data,
                            code,
            ...
            if (xhr.status === 401) {
                ...
                (0, exports.alertError)(msg);
            }
            if (showLoading) {
                (0, exports.setLoading)('close');
            }
            callback({
                data: null,
                code: xhr.status,
...
function clearNullData(params, clearList = [undefined, 'undefined', 'null', '', null]) {
...
function queryToString(query) {
    ...
    Object.keys(query)
        ...
        // 将参数添加到queryList数组中
        const value = query[i];
        if (exports.defaultNullData.includes(value) || value === undefined || value === null) {
            return;
        }
 ...
 */
function getSearchParams(url, type = 'search') {
    // 定义一个空字符串，用于存储查询字符串
    let queryString = '';
    // 如果传入了url参数
    if (url) {
...
function formatCookie(cookieMap) {
    const cookie = [];
